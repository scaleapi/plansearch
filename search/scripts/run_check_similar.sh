...
do
    ...
    do
        python scripts/check_similar.py \
            ...
            --results-file final_results/${dataset}/basic_prompting225_${model}_temp1 \
            --log-directory other_logs/similar_logs/final_logs/${dataset}/basic_prompting225_${model}_temp1 \
            --cache-file caches/${dataset}_${model}_basic_prompting_check_similar_cache.json \
            --max-to-consider 40

        python scripts/check_similar.py \
            --model-config-path model_configs/gpt-4o-mini.json \
            --results-file final_results/${dataset}/simple_idea225_${model}_temp1 \
            --log-directory other_logs/similar_logs/final_logs/${dataset}/simple_idea225_${model}_temp1 \
            --cache-file caches/check_similar_cache.json \
            --max-to-consider 40

        python scripts/check_similar.py \
            --model-config-path model_configs/gpt-4o-mini.json \
            --results-file final_results/${dataset}/combo_observation_no_${model}_temp1 \
            --log-directory other_logs/similar_logs/final_logs/${dataset}/combo_observation_no_${model}_temp1 \
