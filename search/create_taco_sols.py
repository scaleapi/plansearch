...
def main(args: argparse.Namespace):
    ...
    for chunked_problem in tqdm(chunked_problems):
        for codes in try_implementing_problems(chunked_problem, btm, GEN_BATCH_SIZE, NUM_BATCHES_TO_TRY, MAX_NUM_CODES):
            selected_code_sols.append(codes)

    assert len(selected_code_sols) == len(problems)

    new_problems_dataset = {"problem_str": [], "starter_code": [], "tests": [], "generated_solutions": []}

    for problem, selected_codes in zip(problems, selected_code_sols):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-num-codes', type=int, default=120, help='Maximum number of code solutions to output')
